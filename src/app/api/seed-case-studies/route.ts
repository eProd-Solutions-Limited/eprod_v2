...
const CASE_STUDIES = [
    ...
    content: [
      {
        ...
        content: richText(
          ...
          paragraph(
            'eProd designed and deployed a mobile-first Loan Management System tailored to low-connectivity field environments. The system operates fully offline on Android tablets and syncs automatically when a data connection is available — even via 2G.'
          ),
          bulletList([
          ...
          paragraph(
            'Phase 1 (Months 1-2): Discovery, system design, and data migration from legacy spreadsheets. Phase 2 (Months 3-4): Pilot deployment across two branches with 15 loan officers. Phase 3 (Month 5): Full rollout to all 11 branches with a 3-day field training programme.'
          )
        ),
      },
      {
      ...
      },
      {
        blockType: 'stats',
        items: [
          { value: '14 days to 36 hrs', label: 'Loan approval time' },
      ...
      },
      {
        ...
        content: richText(
          heading('The Outcome'),
          paragraph(
            "Within six months of full deployment, AgriCredit Kenya's portfolio at risk (PAR30) dropped from 18% to 6.2%. The organisation was able to onboard 3,400 new borrowers without hiring additional staff, purely through efficiency gains from digitalisation."
          ),
          paragraph(
  ...
  {
    title: 'Unified Digital Banking Platform for a SACCO Federation',
    ...
    content: [
      ...
      {
        blockType: 'quote',
        text: "Our members used to ask why their SACCO couldn't work like a bank. Now we can honestly say: it does, and it does it in a way that keeps them at the centre.",
        author: 'Peter Otieno',
      ...
      },
      {
        ...
        items: [
          ...
          { value: '4.1x', label: 'Increase in cross-SACCO loan uptake' },
      ...
      },
      {
        ...
        content: richText(
          ...
          paragraph(
            'The federation is now piloting an embedded insurance product — micro crop insurance tied to loan disbursements — built directly within the eProd platform. Premiums are deducted automatically at disbursement, with claims processed via satellite crop yield data.'
          )
        ),
      },
    ],
    ...
    content: [
      {
        ...
        content: richText(
          ...
          paragraph(
            "The result: KES 340 million trapped in receivables, unpredictable cash flow, and an inability to grow inventory ahead of planting seasons. FarmLink's CFO described it plainly: we knew we were profitable, but we couldn't pay our own suppliers on time."
          ),
          heading('The eProd Solution', 'h2'),
          paragraph(
            "eProd designed a supply chain finance platform that uses FarmLink's own transaction data — order history, delivery confirmations, payment behaviour — to create dynamic credit risk profiles for each agro-dealer in their network."
          ),
          bulletList([
      ...
      },
      {
        ...
        items: [
          ...
          { value: '31 days', label: 'Avg. collection period (down from 67)' },
      ...
      },
      {
        ...
        content: richText(
          ...
          paragraph(
            'Within 12 months, FarmLink reduced their average collection period from 67 days to 31 days. The partner bank approved a KES 80M invoice discounting facility based directly on the platform receivables data — something they had been unable to secure previously without structured reporting.'
          ),
          paragraph(
...
export async function GET() {
  ...
  for (const study of CASE_STUDIES) {
    ...
    if (existing.docs.length > 0) {
      results.push(`Skipped: "${study.title}" (already exists)`)
      continue
    }
    ...
    results.push(`Created: "${study.title}"`)
  }
